...
Если интернет пропадает только при включённом XKeen после `dns auto on`,
восстановите DNS командой `/opt/sbin/hotwatcher dns auto off` при остановленном
XKeen. Затем проверьте `xkeen -xtest` и запустите `xkeen -start`. Штатный откат
восстанавливает прежний DNS-фрагмент и удаляет только файл, созданный Hot
Watcher. Если URL Test сопровождался сообщением `Out of memory` в `dmesg`,
временный Xray мог превысить RAM роутера. В новой версии экономная проверка
включена по умолчанию и использует основной Xray для URL Test и проверки задержки.
...
Проба проходит через конкретный candidate в основном Xray при включённой экономной
проверке или в отдельном Xray при её отключении. Endpoint может блокироваться
