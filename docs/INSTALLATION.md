...
Начиная с 0.2.7, `hotwatcher stop|start` дополнительно переводят маршрутизацию
между статическим ключом из `04_outbounds.json` и проверенными ключами подписки;
см. [статический режим](STATIC_FALLBACK.md). Команды init.d ниже меняют только процесс.
