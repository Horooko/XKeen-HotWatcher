...
| `selection_policy` | `latency` | Автоматический выбор: `latency` измеряет HTTPS-задержку через VLESS и учитывает пороги смены; `sticky` сохраняет текущий узел |
...
Режим «закреплённый ключ» задаётся в Web UI или командой `hotwatcher pin ИМЯ` и
хранится в приватном состоянии, а не в `config.json`. Он сильнее `selection_policy`:
автоматическая проверка и синхронизация не меняют выбранный ключ даже при ошибке.
`hotwatcher auto` снимает закрепление и запускает автоматический выбор.
