...
func (w *webUI) handler() http.Handler {
	...
	mux.HandleFunc("POST /api/action", func(out http.ResponseWriter, r *http.Request) {
		...
		if input.Action != "sync" && input.Action != "check-key" && input.Action != "url-test" && input.Action != "select" {
			apiError(out, 400, "неизвестная команда")
			return
		}
		if (input.Action == "url-test" || input.Action == "select") && (len(input.Tag) > 128 || strings.ContainsAny(input.Tag, "\r\n")) {
